...
final class ExecuteTool extends Tool
{
    ...
    protected string $description = <<<'TXT'
        Run an executable found by search(), with the user's own permissions. Pass `params` for the page's
        route params and what the executable `needs`: `form` (field values), `row` (a row from query(),
        unchanged) or `selection` (row keys). Invalid input returns validation errors and runs nothing.
        Returns the effects the UI would show (notifications, redirects).
        TXT;

...
| `mcp(array $middleware, string $path = 'mcp'): static` | Expose this panel to AI agents over MCP at `POST {prefix}/{$path}` (needs `composer require laravel/mcp`). $middleware is the route's whole auth and access stack — it REPLACES the panel's middleware, which does not run for MCP calls: name stateless token auth and repeat the panel's role checks, e.g. `['auth:sanctum', 'abilities:tbtop-mcp', 'role:admin']`. A page's own restriction belongs in Page::can(), which MCP enforces. An empty list throws: it would serve the panel without authentication. |

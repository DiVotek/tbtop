...
| `mcp(array $middleware = ['auth:sanctum'], string $path = 'mcp'): static` | Expose this panel to AI agents over MCP at `POST {prefix}/{$path}` (needs `composer require laravel/mcp`). $middleware REPLACES the panel's middleware on that route — use stateless token auth (the default is `auth:sanctum`), and repeat any access check the panel or its pages keep in middleware, since those do not run for MCP calls. |

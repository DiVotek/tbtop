...
| `tabs(array $tabs, array $opts = []): Node` | Tab container. Each tab needs 'body' (a single node) XOR 'children' (a list, optionally laid out via 'columns'), plus a 'label' or 'name' to derive one from. Pass 'name' in $opts to make this a named tabs block (every tab then also needs its own unique 'name', for URL/state addressing). Mark one tab 'active' => true to open it by default instead of the first; when several are marked, the last one wins. |

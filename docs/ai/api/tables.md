...
| `formatUsing(Closure $fn): static` | Server-side formatter, fn(mixed $value): mixed, run in ColumnProjection per row. Runs INSTEAD OF kind-sugar formatting (date/datetime/number/ money), not in addition — set a kind for wire metadata (align, filter type) if needed, but formatUsing() alone decides the displayed value. |

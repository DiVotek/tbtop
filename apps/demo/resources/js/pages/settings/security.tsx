...
export default function Security({ twoFactorEnabled, twoFactorQrSvg, twoFactorSecret, recoveryCodes, passkeys }: SecurityProps) {
    const [showQr, setShowQr] = useState(!!twoFactorQrSvg);
    const [localRecoveryCodes, setLocalRecoveryCodes] = useState<string[] | null>(recoveryCodes ?? null);
    ...
    const confirmSetup: FormEventHandler = (e) => {
        ...
        confirmForm.post(route('two-factor.confirm'), {
            onSuccess: (page) => {
                ...
                const codes = props.recoveryCodes as string[] | undefined;
                if (codes) {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <SettingsLayout>
                <div className="space-y-6">
                    <HeadingSmall title="Two-factor authentication" description="Add extra security to your account" />
                    ...
                    {twoFactorEnabled ? (
                        <div className="space-y-4">
                            ...
                            {localRecoveryCodes && (
                                <div className="space-y-2">
                                    <p className="text-sm font-medium">Recovery codes (save these now — shown once):</p>
                            ...
                            <form onSubmit={disableTwoFactor} className="space-y-4">
                                <div className="grid gap-2">
                                    <Label htmlFor="disable-code">Current OTP or recovery code</Label>
                                    <Input
                                        id="disable-code"
                        ...
                        <div className="space-y-4">
                            {!showQr && <Button onClick={startSetup as unknown as React.MouseEventHandler}>Enable 2FA</Button>}

                            {showQr && (
                                <div className="space-y-4">
                                    <p className="text-sm">Scan this QR code with your authenticator app:</p>
                                    {twoFactorQrSvg && <div dangerouslySetInnerHTML={{ __html: twoFactorQrSvg }} className="max-w-xs" />}
                                    {twoFactorSecret && (
...
function PasskeysSection({ passkeys }: { passkeys: PasskeyItem[] }) {
    ...
    return (
        <div className="space-y-6">
            <HeadingSmall title="Passkeys" description="Sign in without a password using your device's biometrics" />
                        ...
                        <li key={pk.id} className="flex items-center justify-between px-4 py-3">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    {pk.last_used_at && (
                                        <p className="text-muted-foreground text-xs">Last used {new Date(pk.last_used_at).toLocaleDateString()}</p>
                                    )}
                                </div>
                            </div>
                            <Button
            ...
            <div className="flex gap-2">
                <Input
                    ...
                    className="max-w-xs"
                />
                <Button type="button" variant="outline" disabled={isLoading || !passkeyName} onClick={handleRegister}>
